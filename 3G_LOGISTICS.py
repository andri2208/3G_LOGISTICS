...
# --- KONFIGURASI FILE ---
NAMA_FILE = "database_logistik.csv"

def muat_data():
    if os.path.exists(NAMA_FILE):
        df = pd.read_csv(NAMA_FILE)
        # Pastikan kolom angka dibaca sebagai angka, bukan teks kosong
...
# --- FUNGSI TERBILANG ---
def terbilang(n):
    n = int(n) # Memastikan angka bulat
...
tab1, tab2, tab3 = st.tabs(["➕ Input Pengiriman", "📋 Database", "📄 Cetak Invoice"])
...
with tab1:
    with st.form("form_input", clear_on_submit=True):
        ...
        with c1:
            ...
            produk = st.text_input("Deskripsi Produk (Contoh: SATU SET ALAT TAMBANG)")
            origin = st.text_input("Origin (SBY)")
        with c2:
            penerima = st.text_input("Nama Penerima")
            destination = st.text_input("Destination (MEDAN)")
        ...
        if st.form_submit_button("Simpan Data"):
            ...
            st.session_state.df = pd.concat([st.session_state.df, baru], ignore_index=True)
            st.session_state.df.to_csv(NAMA_FILE, index=False)
...
with tab3:
    ...
    if not st.session_state.df.empty:
        ...
        if resi_sel:
            ...
            # Konversi nilai ke float untuk keamanan format
            harga_val = float(d['Harga Satuan'])
            ...
            <div style="border: 1px solid #000; padding: 30px; background-color: white; color: black; font-family: Arial;">
                ...
                <table style="width:100%; border:none;">
                    ...
                    <tr>
                        ...
                        <td style="text-align:right; border:none;">DATE: {d['Tanggal']}</td>
                    </tr>
                </table>
                <br>
                <table style="width:100%; border-collapse: collapse; text-align: center; border: 1px solid black;">
                    ...
                    <tr>
                        ...
                        <td style="border: 1px solid black;">Rp {harga_val:,.0f}</td>
...
<img src="data:image/png;base64,{ttd}" style="width:120px; position: absolute; top: 35px; left: 50%; transform: translateX(-50%);">                        </td>
                    </tr>
                </table>
            </div>
            """, unsafe_allow_html=True)
    else:
        st.info("Belum ada data pengiriman. Silakan input data di Tab 1.")