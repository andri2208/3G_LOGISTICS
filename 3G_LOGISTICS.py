...
def buat_pdf_custom(data):
    ...
    # Detail Customer & Invoice [cite: 3, 4, 5]
    ...
    # Pembayaran & Terbilang [cite: 7, 8, 9, 10, 11]
    pdf.ln(5)
    pdf.set_font("Arial", 'B', 10)
...
# Inisialisasi session state agar tidak NameError
...
with st.container(border=True):
    with st.form("form_input"):
        ...
        submitted = st.form_submit_button("Lihat Preview")
...
# --- 5. HALAMAN PREVIEW ---
# Pastikan data diambil dari session_state sebelum diproses
if st.session_state.preview_data is not None:
    ...
    with st.container(border=True):
        ...
        st.write(f"### Total Bayar: Rp {data_inv['total']:,.0f}") [cite: 8]
        st.write(f"*Terbilang: {terbilang(data_inv['total'])} Rupiah*") [cite: 11]
