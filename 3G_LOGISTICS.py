...
# Fungsi Ekstrak Angka dari Teks (Misal "290 Kg" jadi 290)
...
with tab1:
    ...
    if not data:
        ...
        berat_val = extract_number(row['weight'])
        harga_val = extract_number(row['harga'])
        total_val = int(berat_val * harga_val) if berat_val > 0 else int(harga_val)
        ...
        # DESAIN HTML A5
        html_desain = f"""
        <div id="invoice-a5" style="background-color:white; padding:15px; color:black; font-family:Arial; border:1px solid #eee; width:750px; margin:auto;">
            ...
            <table style="width:100%; border-collapse:collapse; border:1px solid black; font-size:11px; text-align:center;">
                ...
                <tr>
                    ...
                    <td style="border:1px solid black;">Rp {int(harga_val):,}</td>
            ...
            <div style="margin-top:10px; font-size:11px; border:1px solid black; padding:5px;"><b>Terbilang:</b> <i>{teks_terbilang}</i></div>
            <div style="margin-top:20px; display:flex; justify-content:space-between; font-size:11px;">
                <div><b>TRANSFER TO:</b><br>BCA: 6720422334<br>A/N ADITYA GAMA SAPUTRI<br><small>Finance: 082179799200</small></div>
        ...
        <html>
            <body>
                ...
                <button onclick="generatePDF()" style="width:100%; background-color:#28a745; color:white; padding:15px; border:none; border-radius:8px; cursor:pointer; font-weight:bold; font-size:16px;">
                    📥 DOWNLOAD INVOICE (UKURAN A5)
                </button>
                <script>
...
with tab2:
    ...
    with st.form("input_form", clear_on_submit=True):
        ...
        in_kg = c2.text_input("Weight (Angka)")
        ...
        if st.form_submit_button("SIMPAN DATA"):
            ...
            try:
                ...
                st.success(f"Berhasil Simpan! Total: Rp {t_val:,}")
                st.cache_data.clear()
            except:
                st.error("Gagal terhubung ke Database.")
